...
const MAXIMUM_CHUNK_TOKEN_LENGTH = Number.parseInt(
...
const StepTwo = ({
  ...
  const setSegmentIdentifier = useCallback(
    (value: string, canEmpty?: boolean) => {
      doSetSegmentIdentifier(
        value ? escape(value) : canEmpty ? "" : DEFAULT_SEGMENT_IDENTIFIER
      );
    },
    []
  );
  ...
  const [limitMaxChunkLength, setLimitMaxChunkLength] = useState(
  ...
  const [docForm, setDocForm] = useState<ChunkingMode>(
    datasetId && documentDetail
      ? (documentDetail.doc_form as ChunkingMode)
      : ChunkingMode.text
  );
  ...
  const updatePreview = () => {
    if (
      segmentationType === ProcessMode.general &&
      maxChunkLength > MAXIMUM_CHUNK_TOKEN_LENGTH
    ) {
      Toast.notify({
        type: "error",
  ...
  useEffect(() => {
    if (currentDataset?.retrieval_model_dict) return;
    setRetrievalConfig({
      search_method: RETRIEVE_METHOD.semantic,
  ...
  const getCreationParams = () => {
    ...
    if (
      segmentationType === ProcessMode.general &&
      maxChunkLength > limitMaxChunkLength
    ) {
      Toast.notify({
        type: "error",
        message: t("datasetCreation.stepTwo.maxLengthCheck", {
          limit: limitMaxChunkLength,
        }),
      });
      return;
    }
    if (isSetting) {
      ...
      if (dataSourceType === DataSourceType.NOTION)
        params.data_source.info_list.notion_info_list =
          getNotionInfo(notionPages);
  ...
  const fetchDefaultProcessRuleMutation = useFetchDefaultProcessRule({
    onSuccess(data) {
      ...
      setLimitMaxChunkLength(
        data.limits.indexing_max_segmentation_tokens_length
      );
    },
    onError(error) {
      Toast.notify({
  ...
  const isCreating =
    createFirstDocumentMutation.isPending || createDocumentMutation.isPending;
  ...
  const isModelAndRetrievalConfigDisabled =
    !!datasetId && !!currentDataset?.data_source_type;

  return (
    <div className="flex w-full h-full">
      <div
        className={cn(
          "relative h-full w-1/2 py-6 overflow-y-auto",
          isMobile ? "px-4" : "px-12"
        )}
      >
        <div className={"system-md-semibold mb-1"}>
          {t("datasetCreation.stepTwo.segmentation")}
        </div>
        {((isInUpload &&
          [ChunkingMode.text, ChunkingMode.qa].includes(
          ...
          <OptionCard
            ...
            icon={
              <Image
                ...
                height={20}
                src={SettingCog}
                alt={t("datasetCreation.stepTwo.general")}
              />
            }
            activeHeaderClassName="bg-dataset-option-card-blue-gradient"
            description={t("datasetCreation.stepTwo.generalTip")}
            isActive={[ChunkingMode.text, ChunkingMode.qa].includes(
              currentDocForm
            )}
            onSwitched={() => handleChangeDocform(ChunkingMode.text)}
            actions={
              <>
                <Button
                ...
                <div className="mt-1">
                  ...
                  {IS_CE_EDITION && (
                    <>
                      <Divider
                        type="horizontal"
                        className="my-4 bg-divider-subtle"
                      />
                      <div className="flex items-center py-0.5">
                        <div
                          className="flex items-center"
                          onClick={() => {
                            if (currentDataset?.doc_form) return;
                            if (docForm === ChunkingMode.qa)
                              handleChangeDocform(ChunkingMode.text);
                            else handleChangeDocform(ChunkingMode.qa);
                          }}
                        >
                          <Checkbox
                        ...
                        />
                        <Tooltip
                          popupContent={t("datasetCreation.stepTwo.QATip")}
                        />
                      </div>
          ...
          <OptionCard
            ...
            icon={
              <Image
                ...
                height={20}
                src={FamilyMod}
                alt={t("datasetCreation.stepTwo.parentChild")}
              />
            }
                  ...
                  chosenConfig={
                    <div className="flex gap-3">
                      <DelimiterInput
                        ...
                        tooltip={
                          t("datasetCreation.stepTwo.parentChildDelimiterTip")!
                        }
                        onChange={(e) =>
                          setParentChildConfig({
                            ...parentChildConfig,
              ...
              <div>
                ...
                <div className="flex gap-3 mt-1">
                  <DelimiterInput
                    ...
                    tooltip={
                      t("datasetCreation.stepTwo.parentChildChunkDelimiterTip")!
                    }
                    onChange={(e) =>
                      setParentChildConfig({
                        ...parentChildConfig,
        ...
        <div className="flex items-center gap-2">
          {(!hasSetIndexType ||
            (hasSetIndexType && indexingType === IndexingType.QUALIFIED)) && (
            <OptionCard
              className="flex-1"
              title={
          ...
          {(!hasSetIndexType ||
            (hasSetIndexType && indexingType === IndexingType.ECONOMICAL)) && (
            <>
              <CustomDialog
                show={isQAConfirmDialogOpen}
                onClose={() => setIsQAConfirmDialogOpen(false)}
                className="w-[432px]"
              >
              ...
              <PortalToFollowElem
                open={isHoveringEconomy && docForm !== ChunkingMode.text}
                placement={"top"}
              >
        ...
        {indexType === IndexingType.QUALIFIED && (
          <div className="mt-5">
            ...
            <ModelSelector
              ...
              triggerClassName={
                isModelAndRetrievalConfigDisabled ? "opacity-50" : ""
              }
        ...
        <div>
          {!isModelAndRetrievalConfigDisabled ? (
            <div className={"mb-1"}>
              <div className="system-md-semibold mb-0.5">
              ...
              <div className="body-xs-regular text-text-tertiary">
                <a
                  ...
                  href="https://idx-solutions.com/guides/knowledge-base/create-knowledge-and-upload-documents#id-4-retrieval-settings"
                  className="text-text-accent"
                >
      ...
      >
        <PreviewContainer
          header={
            <PreviewHeader title={t("datasetCreation.stepTwo.preview")}>
              <div className="flex items-center gap-1">
                {dataSourceType === DataSourceType.FILE && (
                  <PreviewDocumentPicker
                    ...
                    value={
                      isSetting
                        ? (files[0]! as Required<CustomFile>)
                        : previewFile
                    }
                  />
                )}
                {dataSourceType === DataSourceType.NOTION && (
                  <PreviewDocumentPicker
                    ...
                    onChange={(selected) => {
                      currentEstimateMutation.reset();
                      const selectedPage = notionPages.find(
                        (page) => page.page_id === selected.id
                      );
                ...
                {dataSourceType === DataSourceType.WEB && (
                  <PreviewDocumentPicker
                    ...
                    onChange={(selected) => {
                      currentEstimateMutation.reset();
                      const selectedPage = websitePages.find(
                        (page) => page.source_url === selected.id
                      );
                ...
                {currentDocForm !== ChunkingMode.qa && (
                  <Badge
                    text={
                      t("datasetCreation.stepTwo.previewChunkCount", {
          ...
          className={cn(
            "flex shrink-0 w-1/2 p-4 pr-0 relative h-full",
        ...
        >
          ...
          {currentDocForm === ChunkingMode.parentChild &&
            currentEstimateMutation.data?.preview &&
            estimate?.preview?.map((item, index) => {
