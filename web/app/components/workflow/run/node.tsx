...
const NodePanel: FC<Props> = ({
  ...
  const getTime = (time: number) => {
    ...
    if (time > 60)
      return `${Number.parseInt(Math.round(time / 60).toString())} m ${(
        time % 60
      ).toFixed(3)} s`;
  ...
  const isIterationNode =
    nodeInfo.node_type === BlockEnum.Iteration && !!nodeInfo.details?.length;
  const isLoopNode =
    nodeInfo.node_type === BlockEnum.Loop && !!nodeInfo.details?.length;
  const isRetryNode =
    hasRetryNode(nodeInfo.node_type) && !!nodeInfo.retryDetail?.length;
  const isAgentNode =
    nodeInfo.node_type === BlockEnum.Agent && !!nodeInfo.agentLog?.length;
  const isToolNode =
    nodeInfo.node_type === BlockEnum.Tool && !!nodeInfo.agentLog?.length;
      ...
      <div className="group transition-all bg-background-default border border-components-panel-border rounded-[10px] shadow-xs hover:shadow-md">
        ...
        >
          ...
          <BlockIcon
            ...
            className={cn("shrink-0 mr-2", inMessage && "!mr-1")}
          ...
          {nodeInfo.status !== "running" && !hideInfo && (
            <div className="shrink-0 text-text-tertiary system-xs-regular">
              {nodeInfo.execution_metadata?.total_tokens
                ? `${getTokenCount(
                    nodeInfo.execution_metadata?.total_tokens || 0
                  )} tokens · `
                : ""}
          ...
          {nodeInfo.status === "stopped" && (
            <RiAlertFill
              className={cn(
                "shrink-0 ml-2 w-4 h-4 text-text-warning-secondary",
                inMessage && "w-3.5 h-3.5"
              )}
            />
          )}
            ...
            <div className={cn("mb-1", hideInfo && "!px-2 !py-0.5")}>
              ...
              {nodeInfo.status === "exception" && (
                <StatusContainer status="stopped">
                  {nodeInfo.error}
                  <a
                    href="https://idx-solutions.com/guides/workflow/error-handling/error-type"
                    target="_blank"
            ...
            {nodeInfo.outputs && (
              <div>
                <CodeEditor
                  ...
                  tip={
                    <ErrorHandleTip
                      type={nodeInfo.execution_metadata?.error_strategy}
                    />
                  }
                />
              </div>
