...
export default function VerifyEmail(props: DeleteAccountProps) {
  ...
  const { isPending: isDeleting, mutateAsync: confirmDeleteAccount } =
    useConfirmDeleteAccount();

  useEffect(() => {
    setShouldButtonDisabled(
      !(verificationCode && CODE_EXP.test(verificationCode)) || isDeleting
    );
  }, [verificationCode, isDeleting]);

  const handleConfirm = useCallback(async () => {
    try {
      const ret = await confirmDeleteAccount({
  ...
  return (
    <>
      ...
      <div className="pt-1 pb-2 text-text-secondary body-md-regular">
        ...
        <Link href="https://idx-solutions.com" className="text-text-accent">
          {t("common.account.deletePrivacyLink")}
        </Link>
      </div>
      <label className="mt-3 mb-1 h-6 flex items-center system-sm-semibold text-text-secondary">
        {t("common.account.verificationLabel")}
      </label>
      <Input
        minLength={6}
        maxLength={6}
      ...
      <div className="w-full flex flex-col mt-3 gap-2">
        ...
        >
          {t("common.account.permanentlyDeleteButton")}
        </Button>
