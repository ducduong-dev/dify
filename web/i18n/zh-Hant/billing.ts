const translation = {
  ...
  plansCommon: {
    ...
    vectorSpaceBillingTooltip:
      "向量儲存是將知識庫向量化處理後為讓 LLMs 理解資料而使用的長期記憶儲存，1MB 大約能滿足1.2 million character 的向量化後資料儲存（以 OpenAI Embedding 模型估算，不同模型計算方式有差異）。在向量化過程中，實際的壓縮或尺寸減小取決於內容的複雜性和冗餘性。",
    ...
    supportItems: {
      ...
      llmLoadingBalancingTooltip:
        "向模型添加多個 API 金鑰，從而有效地繞過 API 速率限制。",
    },
    comingSoon: "即將推出",
    member: "成員",
    memberAfter: "個成員",
    ...
    annotatedResponse: {
      title: "標註回覆數",
      tooltip:
        "標註回覆功能透過人工編輯標註為應用提供了可定製的高質量問答回覆能力",
    },
    ragAPIRequestTooltip: "指單獨呼叫 IDX 知識庫資料處理能力的 API。",
