const translation = {
  ...
  datasetUsedByApp:
    "這些知識正被一些應用程序使用。應用程序將無法再使用這些知識，所有提示配置和日誌將被永久刪除。",
  datasetDeleted: "知識庫已刪除",
  datasetDeleteFailed: "刪除知識庫失敗",
  didYouKnow: "你知道嗎？",
  intro1: "知識庫可以被整合到 IDX 應用中",
  intro2: "作為上下文",
  intro3: ",",
  intro4: "或可以",
  intro5: "建立",
  ...
  retrieval: {
    ...
    full_text_search: {
      title: "全文檢索",
      description:
        "索引文件中的所有詞彙，從而允許使用者查詢任意詞彙，並返回包含這些詞彙的文字片段",
    },
    hybrid_search: {
      title: "混合檢索",
      description:
        "同時執行全文檢索和向量檢索，並應用重排序步驟，從兩類查詢結果中選擇匹配使用者問題的最佳結果，需配置 Rerank 模型 API",
      recommend: "推薦",
    },
    invertedIndex: {
      title: "倒排索引",
      description:
        "倒排索引是一種用於高效檢索的結構。按術語組織，每個術語指向包含它的文件或網頁",
    },
    change: "更改",
  ...
  inconsistentEmbeddingModelTip:
    "如果選定知識庫的嵌入模型不一致，則需要重新排序模型。",
  retrievalSettings: "檢索設置",
  rerankSettings: "重新排序設置",
  weightedScore: {
    title: "加權分數",
    description:
      "通過調整分配的權重，此重新排序策略決定是優先考慮語義匹配還是關鍵詞匹配。",
    semanticFirst: "語義優先",
    keywordFirst: "關鍵詞優先",
    customized: "自定義",
    semantic: "語義",
  ...
  allExternalTip:
    "僅使用外部知識時，用戶可以選擇是否啟用 Rerank 模型。如果未啟用，則檢索到的數據塊將根據分數進行排序。當不同知識庫的檢索策略不一致時，就會不準確。",
  ...
  externalAPIPanelDescription:
    "外部知識 API 用於連接到 IDX 外部的知識庫，並從該知識庫中檢索知識。",
  externalAPI: "外部 API",
  ...
  allKnowledgeDescription:
    "選擇以顯示此工作區中的所有知識。只有 Workspace 擁有者可以管理所有知識。",
};
