const translation = {
  ...
  model: {
    params: {
      ...
      maxTokenSettingTip:
        "您設定的最大 tokens 數較大，可能會導致 prompt、使用者問題、知識庫內容沒有 token 空間進行處理，建議設定到 2/3 以下。",
      setToCurrentModelMaxTokenTip:
        "最大令牌數更新為當前模型最大的令牌數 {{maxToken}} 的 80%。",
      stop_sequences: "停止序列 stop_sequences",
      stop_sequencesTip:
        "最多四個序列，API 將停止生成更多的 token。返回的文字將不包含停止序列。",
  ...
  menus: {
    ...
    datasets: "知識庫",
    datasetsTips:
      "即將到來: 上傳自己的長文字資料，或透過 Webhook 整合自己的資料來源",
    newApp: "建立應用",
    newDataset: "建立知識庫",
    tools: "工具",
  ...
  account: {
    ...
    langGeniusAccountTip: "您的 IDX 賬號和相關的使用者資料。",
    editName: "編輯名字",
    ...
    deleteSuccessTip:
      "您的帳戶需要時間才能完成刪除。完成後，我們會給您發送電子郵件。",
  ...
  members: {
    ...
    emailNotSetup:
      "由於郵件伺服器未設置，無法發送邀請郵件。請將邀請後生成的邀請連結通知用戶。",
    email: "郵箱",
    emailInvalid: "郵箱格式無效",
    ...
    invitationSentTip: "邀請已傳送，對方登入 IDX 後即可訪問你的團隊資料。",
    invitationLink: "邀請連結",
  ...
  provider: {
    ...
    openaiHosted: {
      ...
      onTrial: "體驗",
      exhausted: "超出限額",
      desc: "託管 OpenAI 由 IDX 提供的託管 OpenAI 服務，你可以使用 GPT-3.5 等模型，在體驗額度消耗完畢前你需要設定其它模型供應商。",
      callTimes: "呼叫次數",
    ...
    anthropicHosted: {
      ...
      close: "關閉",
      trialQuotaTip:
        "您的 Anthropic 試用配額將於 2025 年 3 月 11 日到期，此後將不再可用。請及時利用。",
    },
    anthropic: {
      using: "嵌入能力正在使用",
      enableTip:
        "要啟用 Anthropic 模型，您需要先繫結 OpenAI 或 Azure OpenAI 服務。",
      notEnabled: "未啟用",
  ...
  modelProvider: {
    ...
    item: {
      deleteDesc:
        "{{modelName}} 被用作系統推理模型。刪除後部分功能將無法使用。請確認。",
      freeQuota: "免費額度",
    },
    ...
    editConfig: "編輯配置",
    loadBalancingInfo:
      "默認情況下，負載均衡使用 Round-robin 策略。如果觸發了速率限制，將應用 1 分鐘的冷卻時間。",
  ...
  apiBasedExtension: {
    title:
      "API 擴充套件提供了一個集中式的 API 管理，在此統一新增 API 配置後，方便在 IDX 上的各類應用中直接使用。",
    link: "瞭解如何開發您自己的 API 擴充套件。",
    linkUrl:
      "https://idx-solutions.com/v/zh-hans/guides/extension/api-based-extension",
    add: "新增 API 擴充套件",
    selector: {
      title: "API 擴充套件",
