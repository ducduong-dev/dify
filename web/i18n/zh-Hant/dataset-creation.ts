const translation = {
  ...
  stepOne: {
    ...
    website: {
      ...
      maxDepthTooltip:
        "相對於輸入的 URL 的最大爬網深度。深度 0 只是抓取輸入的 url 的頁面，深度 1 抓取 url 以及 enteredURL + 1 / 之後的所有內容，依此類推。",
      jinaReaderNotConfigured: "未配置 Jina Reader",
      jinaReaderNotConfiguredDescription:
        "通過輸入您的免費 API 金鑰進行訪問來設置 Jina Reader。",
      ...
      useSitemapTooltip:
        "按照網站地圖對網站進行爬網。否則，Jina Reader 將根據頁面相關性反覆運算抓取，從而生成更少但品質更高的頁面。",
    },
  },
  stepTwo: {
    segmentation: "分段設定",
    auto: "自動分段與清洗",
    autoDescription:
      "自動設定分段規則與預處理規則，如果不瞭解這些引數建議選擇此項",
    custom: "自定義",
    ...
    overlap: "分段重疊長度",
    overlapTip:
      "設定分段之間的重疊長度可以保留分段之間的語義關係，提升召回效果。建議設定為最大分段長度的10%-25%",
    ...
    recommend: "推薦",
    qualifiedTip:
      "呼叫系統預設的嵌入介面進行處理，以在使用者查詢時提供更高的準確度",
    ...
    sideTipP2:
      "分段的目的是將長文字拆分成較小的段落，以便模型更有效地處理和理解。這有助於提高模型生成的結果的質量和相關性。",
    sideTipP3:
      "清洗則是對文字進行預處理，刪除不必要的字元、符號或格式，使知識庫更加乾淨、整潔，便於模型解析。",
    sideTipP4:
      "透過對知識庫進行適當的分段和清洗，可以提高模型在實際應用中的表現，從而為使用者提供更準確、更有價值的結果。",
    previewTitle: "分段預覽",
    ...
    qaSwitchHighQualityTipContent:
      "目前，只有高品質索引方法支援Q&A格式分塊。是否要切換到高品質模式？",
    previewChunk: "預覽資料塊（Preview Chunk）",
    fullDocTip:
      "整個文件用作父塊並直接檢索。請注意，出於性能原因，超過10000個令牌的文本將被自動截斷。",
    ...
    parentChildDelimiterTip:
      "分隔符是用於分隔文字的字元。建議將原始文檔拆分為多個大型父塊。您還可以使用自己定義的特殊分隔符。",
    generalTip: "常規文本分塊模式，檢索和調用的塊是相同的。",
    highQualityTip:
      "在 High Quality 模式下完成嵌入後，將無法恢復到 Economical （經濟） 模式。",
    childChunkForRetrieval: "用於檢索的 Child-chunk",
    paragraphTip:
      "此模式根據分隔符和最大區塊長度將文本拆分為段落，使用拆分文本作為父區塊進行檢索。",
    paragraph: "段",
    qaSwitchHighQualityTipTitle: "Q&A 格式需要高品質的索引方法",
    notAvailableForParentChild: "不適用於父子索引",
    fullDoc: "完整文件",
    parentChildTip:
      "當使用 parent-child 模式時，child-chunk 用於檢索，parent-chunk 用於作為上下文調用 call。",
    switch: "開關",
  },
  stepThree: {
    ...
    sideTipTitle: "接下來做什麼",
    sideTipContent:
      "當文件完成索引處理後，知識庫即可整合至應用內作為上下文使用，你可以在提示詞編排頁找到上下文設定。你也可以建立成可獨立使用的 ChatGPT 索引外掛釋出。",
  ...
  otherDataSource: {
    learnMore: "瞭解更多資訊",
    description:
      "目前，Dify 的知識庫只有有限的數據源。向 IDX 知識庫貢獻數據源是説明所有使用者增強平台靈活性和強大功能的絕佳方式。我們的貢獻指南使入門變得容易。請點擊下面的連結瞭解更多資訊。",
    title: "連接到其他數據源？",
  },
};
