const translation = {
  ...
  apiKeyInfo: {
    cloud: {
      trial: {
        ...
        description:
          "試用配額僅供您測試使用。 在試用配額用完之前，請自行設定模型提供商或購買額外配額。",
      },
      exhausted: {
    ...
    usedToken: "使用 Tokens",
    setAPIBtn: "設定模型提供商",
    tryCloud: "或者嘗試使用 IDX 的雲版本並使用試用配額",
  },
  overview: {
    title: "概覽",
    appInfo: {
      ...
      settings: {
        ...
        more: {
          ...
          privacyPolicyTip:
            "幫助訪問者瞭解該應用收集的資料，可參考 IDX 的<privacyPolicyLink>隱私政策</privacyPolicyLink>。",
          customDisclaimer: "自定義免責聲明",
          customDisclaimerPlaceholder: "請輸入免責聲明",
          customDisclaimerTip:
            "客製化的免責聲明文字將在客戶端顯示，提供有關應用程式的額外資訊。",
      ...
      embedded: {
        ...
        chromePlugin: "安裝 IDX Chrome 瀏覽器擴充套件",
        copied: "已複製",
        copy: "複製",
      },
      qrcode: {
  ...
  analysis: {
    ...
    totalMessages: {
      title: "全部訊息數",
      explanation:
        "反映 AI 每天的互動總次數，每回答使用者一個問題算一條 Message。提示詞編排和除錯的訊息不計入。",
    },
    activeUsers: {
      title: "活躍使用者數",
    ...
    avgSessionInteractions: {
      title: "平均會話互動數",
      explanation:
        "反映每個會話使用者的持續溝通次數，如果使用者與 AI 問答了 10 輪，即為 10。該指標反映了使用者粘性。僅在對話型應用提供。",
    },
    avgUserInteractions: {
      title: "平均使用者呼叫次數",
    ...
    avgResponseTime: {
      title: "平均響應時間",
      explanation:
        "衡量 AI 應用處理和回覆使用者請求所花費的平均時間，單位為毫秒，反映效能和使用者體驗。僅在文字型應用提供。",
    },
    tps: {
      title: "Token 輸出速度",
      explanation:
        "衡量 LLM 的效能。統計 LLM 從請求開始到輸出完畢這段期間的 Tokens 輸出速度。",
    },
  },
};
