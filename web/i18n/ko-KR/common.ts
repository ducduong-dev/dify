const translation = {
  ...
  model: {
    params: {
      temperature: "온도",
      temperatureTip:
        "랜덤성을 제어합니다. 온도를 낮추면 더 랜덤한 결과물을 얻을 수 있습니다. 온도가 0에 가까워질수록 모델은 결정적이고 반복적으로 작동합니다.",
      top_p: "상위P",
      top_pTip:
        "뉴클리어스 샘플링에 의한 다양성 제어: 0.5는 모든 확률 가중 옵션의 절반을 고려함을 의미합니다.",
      presence_penalty: "존재 페널티",
      presence_penaltyTip:
        "이전 텍스트에서 토큰이 나타나는지 여부에 따라 새로운 토큰에 얼마나 많은 페널티를 부여할지 제어합니다. 모델이 새로운 주제에 대해 말할 가능성이 높아집니다.",
      ...
      maxTokenSettingTip:
        "최대 토큰 설정이 높아서 프롬프트, 쿼리 및 데이터 공간에 제한이 생길 수 있습니다. 현재 모델의 최대 토큰의 80% 이하로 설정해주세요.",
      setToCurrentModelMaxTokenTip:
        "최대 토큰이 현재 모델의 최대 토큰의 80%로 업데이트되었습니다 {{maxToken}}.",
      stop_sequences: "중단 시퀀스",
      stop_sequencesTip:
        "API가 진행 중인 토큰 생성을 중단하는 최대 4개의 시퀀스입니다. 반환된 텍스트에는 중단 시퀀스가 포함되지 않습니다.",
  ...
  menus: {
    ...
    plugins: "플러그인",
    pluginsTips:
      "타사 플러그인을 통합하거나 ChatGPT 호환 AI 플러그인을 작성합니다.",
    datasets: "지식",
    datasetsTips:
      "곧 출시될 예정: 고유한 텍스트 데이터를 가져오거나 웹훅을 통해 실시간으로 데이터를 기록하여 LLM 컨텍스트를 강화합니다.",
    newApp: "새로운 앱",
    newDataset: "지식 만들기",
    tools: "도구",
  ...
  account: {
    ...
    password: "비밀번호",
    passwordTip:
      "일시적인 로그인 코드를 사용하지 않으려면 영구적인 비밀번호를 설정할 수 있습니다.",
    setPassword: "비밀번호 설정",
    ...
    delete: "계정 삭제",
    deleteTip:
      "계정을 삭제하면 모든 데이터가 영구적으로 지워지며 복구할 수 없습니다.",
    ...
    deleteSuccessTip:
      "계정 삭제를 완료하는 데 시간이 필요합니다. 모든 작업이 완료되면 이메일로 연락드리겠습니다.",
    ...
    deletePrivacyLinkTip:
      "당사가 귀하의 데이터를 처리하는 방법에 대한 자세한 내용은 다음을 참조하십시오.",
  },
  members: {
    team: "팀",
    invite: "초대",
    ...
    emailNotSetup:
      "이메일 서버가 설정되지 않아 초대 이메일을 보낼 수 없습니다. 대신 초대 후 발급되는 초대 링크를 사용자에게 알려주세요.",
    email: "이메일",
    ...
    invitationSentTip:
      "초대가 전송되었으며, 그들은 Dify에 로그인하여 당신의 팀 데이터에 액세스할 수 있습니다.",
  ...
  provider: {
    ...
    apiKeyExceedBill:
      "이 API KEY에는 사용 가능한 할당량이 없습니다. 자세한 내용은",
    addKey: "키 추가",
    comingSoon: "곧 출시됨",
    editKey: "편집",
    ...
    openaiHosted: {
      ...
      callTimes: "호출 횟수",
      usedUp:
        "트라이얼 할당량이 다 사용되었습니다. 다른 모델 제공자를 추가하세요.",
    ...
    anthropicHosted: {
      ...
      close: "닫기",
      trialQuotaTip:
        "Anthropic 평가판 할당량은 2025/03/11에 만료되며 그 이후에는 더 이상 사용할 수 없습니다. 제때 활용하시기 바랍니다.",
    },
    anthropic: {
      using: "임베드 기능을 사용 중입니다",
      enableTip:
        "Anthropic 모델을 활성화하려면 먼저 OpenAI 또는 Azure OpenAI 서비스에 바인딩해야 합니다.",
      notEnabled: "비활성화됨",
  ...
  modelProvider: {
    notConfigured:
      "시스템 모델이 아직 완전히 설정되지 않아 일부 기능을 사용할 수 없습니다.",
    ...
    item: {
      deleteDesc:
        "{{modelName}}은(는) 시스템 추론 모델로 사용 중입니다. 제거 후 일부 기능을 사용할 수 없습니다. 확인하시겠습니까?",
      freeQuota: "무료 할당량",
    },
    addApiKey: "API 키 추가",
    ...
    upgradeForLoadBalancing:
      "로드 밸런싱을 사용하도록 계획을 업그레이드합니다.",
    apiKeyRateLimit:
      "속도 제한에 도달했으며, {{seconds}}s 후에 사용할 수 있습니다.",
    loadBalancingInfo:
      "기본적으로 부하 분산은 라운드 로빈 전략을 사용합니다. 속도 제한이 트리거되면 1분의 휴지 기간이 적용됩니다.",
    loadBalancingLeastKeyWarning:
      "로드 밸런싱을 사용하려면 최소 2개의 키를 사용하도록 설정해야 합니다.",
    providerManagedDescription:
      "모델 공급자가 제공하는 단일 자격 증명 집합을 사용합니다.",
  ...
  apiBasedExtension: {
    title:
      "API 기반 확장은 IDX 애플리케이션 전체에서 간편한 사용을 위한 설정을 단순화하고 집중적인 API 관리를 제공합니다.",
  ...
  datasetMenus: {
    ...
    settings: "설정",
    emptyTip:
      "연결된 지식이 없습니다. 애플리케이션 또는 플러그인으로 이동하여 연결을 완료하세요.",
    viewDoc: "문서 보기",
    relatedApp: "관련 앱",
  ...
  promptEditor: {
    placeholder:
      '여기에 프롬프트 단어를 입력하세요. 변수를 삽입하려면 "{{"를 입력하고, 프롬프트 컨텐츠 블록을 삽입하려면 "/"를 입력하세요.',
    context: {
      item: {
        title: "컨텍스트",
  ...
  imageUploader: {
    ...
    uploadFromComputerLimit:
      "업로드 이미지 크기는 {{size}} MB를 초과할 수 없습니다",
