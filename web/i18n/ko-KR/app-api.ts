const translation = {
  ...
  apiKeyModal: {
    ...
    apiSecretKeyTips:
      "API 키를 보호하여 API의 남용을 방지하십시오. 프런트엔드 코드에서 평문으로 사용하지 마세요. :)",
  ...
  completionMode: {
    title: "완성 모드 API",
    info: "문서, 요약, 번역 등 고품질 텍스트 생성을 위해 사용자 입력을 사용하는 완성 메시지 API를 사용합니다. 텍스트 생성은 IDX Prompt Engineering에서 설정한 모델 매개변수와 프롬프트 템플릿에 의존합니다.",
    createCompletionApi: "완성 메시지 생성",
    createCompletionApiTip:
      "질의 응답 모드를 지원하기 위해 완성 메시지를 생성합니다.",
    inputsTips:
    ...
    blocking:
      "블로킹 유형으로 실행이 완료되고 결과가 반환될 때까지 대기합니다. (처리가 오래 걸리면 요청이 중단될 수 있습니다)",
    streaming:
      "스트리밍 반환. SSE(Server-Sent Events)를 기반으로 하는 스트리밍 반환 구현.",
    messageFeedbackApi: "메시지 피드백(좋아요)",
    messageFeedbackApiTip:
      '엔드 사용자 대신 수신된 메시지를 "좋아요" 또는 "좋아요"로 평가합니다. 이 데이터는 로그 및 주석 페이지에 표시되며 향후 모델 세부 조정에 사용됩니다.',
    messageIDTip: "메시지 ID",
    ratingTip: "좋아요 또는 좋아요, null은 취소",
    parametersApi: "애플리케이션 매개변수 정보 가져오기",
    parametersApiTip:
      "변수 이름, 필드 이름, 유형, 기본값을 포함한 설정된 입력 매개변수를 가져옵니다. 일반적으로 이러한 필드는 양식에 표시하거나 클라이언트 로드 후에 기본값을 입력하는 데 사용됩니다.",
  },
  chatMode: {
    title: "채팅 모드 API",
    info: "질의 응답 형식을 사용하는 다목적 대화형 응용 프로그램에는 채팅 메시지 API를 호출하여 대화를 시작합니다. 반환된 conversation_id를 전달하여 계속된 대화를 유지합니다. 응답 매개변수 및 템플릿은 IDX Prompt Eng의 설정에 의존합니다.",
    ...
    streaming:
      "스트리밍 반환. SSE(Server-Sent Events)를 기반으로 하는 스트리밍 반환 구현.",
    conversationIdTip:
      "(선택 사항) 대화 ID: 처음 대화의 경우 비워두고, 계속된 경우 컨텍스트에서 conversation_id를 전달합니다.",
    messageFeedbackApi: "메시지 피드백(좋아요)",
    messageFeedbackApiTip:
      '엔드 사용자 대신 수신된 메시지를 "좋아요" 또는 "좋아요"로 평가합니다. 이 데이터는 로그 및 주석 페이지에 표시되며 향후 모델 세부 조정에 사용됩니다.',
    messageIDTip: "메시지 ID",
