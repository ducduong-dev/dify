const translation = {
  ...
  stepOne: {
    ...
    website: {
      ...
      exceptionErrorTitle:
        "Firecrawl 작업을 실행하는 동안 예외가 발생했습니다.",
      scrapTimeInfo: "{{time}}s 내에 총 {{total}} 페이지를 스크랩했습니다.",
      unknownError: "알 수 없는 오류",
      totalPageScraped: "스크랩한 총 페이지 수:",
      fireCrawlNotConfiguredDescription:
        "API 키로 Firecrawl을 구성하여 사용합니다.",
      extractOnlyMainContent:
        "기본 콘텐츠만 추출합니다(머리글, 탐색, 바닥글 등 없음).",
      maxDepthTooltip:
        "입력한 URL을 기준으로 크롤링할 최대 수준입니다. 깊이 0은 입력 된 url의 페이지를 긁어 내고, 깊이 1은 url과 enteredURL + one / 이후의 모든 것을 긁어 모으는 식입니다.",
      chooseProvider: "제공자 선택",
      jinaReaderDocLink: "https://jina.ai/reader",
      useSitemap: "사이트맵 사용",
      jinaReaderNotConfiguredDescription:
        "액세스를 위해 무료 API 키를 입력하여 Jina Reader를 설정합니다.",
      ...
      useSitemapTooltip:
        "사이트맵을 따라 사이트를 크롤링합니다. 그렇지 않은 경우 Jina Reader는 페이지 관련성에 따라 반복적으로 크롤링하여 더 적지만 더 높은 품질의 페이지를 생성합니다.",
    },
    cancel: "취소",
  },
  stepTwo: {
    ...
    auto: "자동",
    autoDescription:
      "청크 및 전처리 규칙을 자동으로 설정합니다. 처음 사용자는 이 옵션을 선택하는 것을 권장합니다.",
    custom: "사용자 설정",
    customDescription:
    ...
    overlap: "청크 중첩",
    overlapTip:
      "청크 중첩을 설정하여 그 사이의 의미적 연관성을 유지하고 검색 효과를 향상시킬 수 있습니다. 최대 청크 크기의 10%~25%로 설정하는 것이 좋습니다.",
    ...
    recommend: "추천",
    qualifiedTip:
      "사용자 쿼리에 대해 더 높은 정확성을 제공하기 위해 기본 시스템 임베딩 인터페이스를 호출하여 처리합니다.",
    warning: "모델 제공자의 API 키를 설정하세요.",
    click: "설정으로 이동",
    economical: "경제적",
    economicalTip:
      "오프라인 벡터 엔진, 키워드 인덱스 등을 사용하여 토큰 소비 없이 정확도를 낮춥니다.",
    ...
    sideTipP1:
      "텍스트 데이터를 처리할 때 청크와 클리닝은 두 가지 중요한 전처리 단계입니다.",
    sideTipP2:
      "세그멘테이션은 긴 텍스트를 단락으로 분할하여 모델이 이해하기 쉽게 합니다. 이로 인해 모델 결과의 품질과 관련성이 향상됩니다.",
    sideTipP3:
      "클리닝은 불필요한 문자 및 형식을 제거하여 지식을 더 깔끔하고 분석 가능한 것으로 만듭니다.",
    sideTipP4:
      "적절한 청크와 클리닝은 모델의 성능을 향상시키고 정확하고 가치 있는 결과를 제공합니다.",
    previewTitle: "미리보기",
    previewTitleButton: "미리보기",
    previewButton: "질문-답변 형식으로 전환",
    previewSwitchTipStart:
      "현재 청크 미리보기는 텍스트 형식입니다. 질문과 답변 형식 미리보기로 전환하면",
    ...
    highQualityTip:
      "고품질 모드에서 삽입을 마치면 경제적 모드로 되돌릴 수 없습니다.",
    notAvailableForQA: "Q&A 인덱스에는 사용할 수 없습니다.",
    qaSwitchHighQualityTipTitle:
      "Q&A 형식에는 고품질 인덱싱 방법이 필요합니다.",
    notAvailableForParentChild: "부모-자식 인덱스에는 사용할 수 없습니다.",
    previewChunkCount: "{{개수}} 추정된 청크",
    parentChildTip:
      "부모-자식 모드를 사용할 때 자식 청크는 검색에 사용되고 부모 청크는 컨텍스트로 회수에 사용됩니다.",
    generalTip:
      "일반적인 텍스트 청크 모드에서는 검색된 청크와 회수된 청크가 동일합니다.",
    fullDocTip:
      "전체 문서가 상위 청크로 사용되며 직접 검색됩니다. 성능상의 이유로 10000 토큰을 초과하는 텍스트는 자동으로 잘립니다.",
    parentChildChunkDelimiterTip:
      "구분 기호는 텍스트를 구분하는 데 사용되는 문자입니다. \\n 은 부모 청크를 작은 자식 청크로 분할하는 데 권장됩니다. 직접 정의한 특수 구분 기호를 사용할 수도 있습니다.",
    switch: "스위치",
    paragraphTip:
      "이 모드는 구분 기호와 최대 청크 길이에 따라 텍스트를 단락으로 분할하며, 분할된 텍스트를 검색을 위한 부모 청크로 사용합니다.",
  },
  stepThree: {
    creationTitle: "🎉 지식이 생성되었습니다",
    creationContent:
      "지식 이름이 자동으로 설정되었지만 언제든지 변경할 수 있습니다",
    label: "지식 이름",
    ...
    modelContent:
      "나중에 처리를 다시 시작해야 할 경우, 중단한 위치에서 계속합니다.",
  ...
  otherDataSource: {
    ...
    description:
      "현재 Dify의 기술 자료에는 제한된 데이터 소스만 있습니다. IDX 기술 자료에 데이터 소스를 제공하는 것은 모든 사용자를 위해 플랫폼의 유연성과 기능을 향상시키는 데 도움이 되는 환상적인 방법입니다. 기여 가이드를 통해 쉽게 시작할 수 있습니다. 자세한 내용은 아래 링크를 클릭하십시오.",
  },
};
