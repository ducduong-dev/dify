const translation = {
  ...
  model: {
    params: {
      ...
      maxTokenSettingTip:
        "Ваша настройка максимального количества токенов высока, что потенциально ограничивает пространство для подсказок, запросов и данных. Подумайте о том, чтобы установить его ниже 2/3.",
      setToCurrentModelMaxTokenTip:
        "Максимальное количество токенов обновлено до 80% максимального количества токенов текущей модели {{maxToken}}.",
      stop_sequences: "Стоп-последовательности",
      stop_sequencesTip:
        "До четырех последовательностей, где API прекратит генерировать дальнейшие токены. Возвращаемый текст не будет содержать стоп-последовательность.",
  ...
  menus: {
    ...
    plugins: "Плагины",
    pluginsTips:
      "Интегрируйте сторонние плагины или создавайте совместимые с ChatGPT AI-плагины.",
    datasets: "Знания",
    datasetsTips:
      "СКОРО: Импортируйте свои собственные текстовые данные или записывайте данные в режиме реального времени через Webhook для улучшения контекста LLM.",
  ...
  account: {
    ...
    langGeniusAccountTip:
      "Ваша учетная запись IDX и связанные с ней пользовательские данные.",
    editName: "Редактировать имя",
    showAppLength: "Показать {{length}} приложений",
    delete: "Удалить учетную запись",
    deleteTip:
      "Удаление вашей учетной записи приведет к безвозвратному удалению всех ваших данных, и их невозможно будет восстановить.",
    deleteConfirmTip:
      "Для подтверждения, пожалуйста, отправьте следующее с вашего зарегистрированного адреса электронной почты на ",
    account: "Счет",
    studio: "Студия Dify",
    ...
    deleteLabel:
      "Для подтверждения, пожалуйста, введите свой адрес электронной почты ниже",
    deleteSuccessTip:
      "Вашему аккаунту требуется время, чтобы завершить удаление. Мы свяжемся с вами по электронной почте, когда все будет готово.",
    deletePrivacyLinkTip:
      "Для получения дополнительной информации о том, как мы обрабатываем ваши данные, ознакомьтесь с нашим",
  },
  members: {
    team: "Команда",
    invite: "Добавить",
    ...
    normal: "Обычный",
    normalTip:
      "Может только использовать приложения, не может создавать приложения",
    builder: "Разработчик",
    ...
    inviteTeamMemberTip:
      "Они могут получить доступ к данным вашей команды сразу после входа в систему.",
    emailNotSetup:
      "Почтовый сервер не настроен, поэтому приглашения по электронной почте не могут быть отправлены. Пожалуйста, уведомите пользователей о ссылке для приглашения, которая будет выдана после приглашения.",
    email: "Электронная почта",
    ...
    invitationSentTip:
      "Приглашение отправлено, и они могут войти в Dify, чтобы получить доступ к данным вашей команды.",
  ...
  provider: {
    ...
    apiKeyExceedBill:
      "Этот API-ключ не имеет доступной квоты, пожалуйста, прочитайте",
    ...
    azure: {
      apiBase: "Базовый API",
      apiBasePlaceholder:
        "Базовый URL-адрес API вашей конечной точки Azure OpenAI.",
      apiKey: "Ключ API",
    ...
    openaiHosted: {
      ...
      usedUp:
        "Пробная квота исчерпана. Добавьте собственного поставщика модели.",
      useYourModel:
        "В настоящее время используется собственный поставщик модели.",
      close: "Закрыть",
    },
    anthropicHosted: {
      ...
      close: "Закрыть",
      trialQuotaTip:
        "Ваша квота на пробную версию Anthropic истечет 11.03.2025 и больше не будет доступна. Пожалуйста, используйте его вовремя.",
    },
    anthropic: {
      using: "Возможность встраивания использует",
      enableTip:
        "Чтобы включить модель Anthropic, вам необходимо сначала привязаться к OpenAI или Azure OpenAI Service.",
  ...
  modelProvider: {
    notConfigured:
      "Системная модель еще не полностью настроена, и некоторые функции могут быть недоступны.",
    ...
    item: {
      deleteDesc:
        "{{modelName}} используются в качестве моделей системного мышления. Некоторые функции будут недоступны после удаления. Пожалуйста, подтвердите.",
    ...
    loadBalancingDescription:
      "Снизьте нагрузку с помощью нескольких наборов учетных данных.",
    ...
    providerManagedDescription:
      "Используйте один набор учетных данных, предоставленный поставщиком модели.",
    ...
    loadBalancingLeastKeyWarning:
      "Для включения балансировки нагрузки необходимо включить не менее 2 ключей.",
    loadBalancingInfo:
      "По умолчанию балансировка нагрузки использует стратегию Round-robin. Если срабатывает ограничение скорости, будет применен 1-минутный период охлаждения.",
    upgradeForLoadBalancing:
      "Обновите свой тарифный план, чтобы включить балансировку нагрузки.",
  ...
  dataSource: {
    ...
    notion: {
      title: "Notion",
      description:
        "Использование Notion в качестве источника данных для знаний.",
  ...
  apiBasedExtension: {
    title:
      "API-расширения обеспечивают централизованное управление API, упрощая настройку для удобного использования в приложениях Dify.",
  ...
  datasetMenus: {
    ...
    settings: "Настройки",
    emptyTip:
      "Знания не были связаны, пожалуйста, перейдите в приложение или плагин, чтобы завершить связывание.",
  ...
  promptEditor: {
    placeholder:
      "Напишите здесь свое ключевое слово подсказки, введите '{', чтобы вставить переменную, введите '/', чтобы вставить блок содержимого подсказки",
    context: {
      item: {
        title: "Контекст",
  ...
  imageUploader: {
    ...
    uploadFromComputerReadError:
      "Ошибка чтения изображения, повторите попытку.",
    uploadFromComputerUploadError:
      "Ошибка загрузки изображения, загрузите еще раз.",
    uploadFromComputerLimit:
      "Загружаемые изображения не могут превышать {{size}} МБ",
  ...
  fileUploader: {
    ...
    uploadFromComputerReadError:
      "Чтение файла не удалось, пожалуйста, повторите попытку.",
    pasteFileLinkInvalid: "Неверная ссылка на файл",
    uploadFromComputerLimit: "Файл загрузки не может превышать {{size}}",
    uploadFromComputerUploadError:
      "Загрузка файла не удалась, пожалуйста, загрузите еще раз.",
  },
  license: {
