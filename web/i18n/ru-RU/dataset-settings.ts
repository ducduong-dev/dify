const translation = {
  ...
  form: {
    ...
    descInfo:
      "Пожалуйста, напишите четкое текстовое описание, чтобы обрисовать содержание базы знаний. Это описание будет использоваться в качестве основы для сопоставления при выборе из нескольких баз знаний для вывода.",
    descPlaceholder:
      "Опишите, что находится в этой базе знаний. Подробное описание позволяет ИИ своевременно получать доступ к содержимому базы знаний. Если оставить пустым, IDX будет использовать стратегию поиска по умолчанию.",
    ...
    indexMethodHighQualityTip:
      "Вызов модели встраивания для обработки, чтобы обеспечить более высокую точность при запросах пользователей.",
    ...
    retrievalSetting: {
      ...
      longDescription:
        " о методе поиска, вы можете изменить это в любое время в настройках базы знаний.",
    },
    save: "Сохранить",
    ...
    upgradeHighQualityTip:
      "После обновления до режима «Высокое качество» возврат к экономичному режиму невозможен",
    indexMethodChangeToEconomyDisabledTip:
      "Недоступно для понижения уровня с HQ до ECO",
  },
};
