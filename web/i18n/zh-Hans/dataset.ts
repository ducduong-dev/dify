const translation = {
  ...
  externalAPIPanelDescription:
    "外部知识库 API 用于连接到 IDX 之外的知识库并从中检索知识。",
  ...
  datasetUsedByApp:
    "某些应用正在使用该知识库。应用将无法再使用该知识库,所有的提示配置和日志将被永久删除。",
  ...
  mixtureHighQualityAndEconomicTip:
    "混合使用高质量和经济型知识库需要配置 Rerank 模型。",
  inconsistentEmbeddingModelTip:
    "当所选知识库配置的 Embedding 模型不一致时，需要配置 Rerank 模型。",
  mixtureInternalAndExternalTip:
    "混合使用内部和外部知识时需要配置 Rerank 模型。",
  allExternalTip:
    "仅使用外部知识时，用户可以选择是否启用 Rerank 模型。如果不启用，检索到的文本块将根据分数排序。当不同知识库的检索策略不一致时，结果可能不准确。",
  ...
  nTo1RetrievalLegacy:
    "9 月 1 日起我们将不再提供此能力，推荐使用最新的多路召回获得更好的检索效果。",
  ...
  allKnowledgeDescription:
    "选择以显示该工作区内所有知识库。只有工作区所有者才能管理所有知识库。",
};
