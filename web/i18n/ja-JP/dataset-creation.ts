const translation = {
  ...
  stepOne: {
    ...
    uploader: {
      ...
      validation: {
        ...
        filesNumber:
          "バッチアップロードの制限（{{filesNumber}}個）に達しました。",
      },
      cancel: "キャンセル",
      change: "変更",
    ...
    website: {
      ...
      fireCrawlNotConfiguredDescription:
        "Firecrawl を使用するには、Firecrawl の API キーを設定してください。",
      configure: "設定",
      run: "実行",
      ...
      extractOnlyMainContent:
        "メインコンテンツのみを抽出する(ヘッダー、ナビ、フッターなどは抽出しない)",
      ...
      resetAll: "すべてリセット",
      scrapTimeInfo:
        "{{time}} 秒以内に合計 {{total}} ページをスクレイピングしました",
      preview: "プレビュー",
      maxDepthTooltip:
        '入力されたURLを基にしたクローリング作業での設定可能な最大深度について説明します。深度0は入力されたURL自体のページを対象としたスクレイピングを意味します。深度1では、元のURLの直下にあるページ（URLに続く最初の"/"以降の内容）もスクレイピングの対象になります。この深度は指定した数値まで増加させることができ、それに応じてスクレイピングの範囲も広がっていきます。',
      ...
      jinaReaderNotConfiguredDescription:
        "無料のAPIキーを入力してJina Readerを設定します。",
      useSitemapTooltip:
        "サイトマップに沿ってサイトをクロールします。そうでない場合、Jina Readerはページの関連性に基づいて繰り返しクロールし、ページ数は少なくなりますが、高品質のページが得られます。",
    },
  },
  stepTwo: {
    segmentation: "チャンク設定",
    ...
    custom: "カスタム",
    customDescription:
      "チャンクのルール、チャンクの長さ、前処理ルールなどをカスタマイズします。",
    general: "汎用",
    generalTip:
    ...
    overlapTip:
      "チャンクのオーバーラップを設定することで、それらの間の意味的な関連性を維持し、検索効果を向上させることができます。最大チャンクサイズの10%〜25%を設定することをおすすめします。",
    overlapCheck:
      "チャンクのオーバーラップは最大チャンク長を超えてはいけません",
    rules: "テキストの前処理ルール",
    ...
    sideTipP1:
      "テキストデータを処理する際、チャンクとクリーニングは2つの重要な前処理ステップです。",
    sideTipP2:
      "セグメンテーションは長いテキストを段落に分割し、モデルがより理解しやすくします。これにより、モデルの結果の品質と関連性が向上します。",
    sideTipP3:
      "クリーニングは不要な文字や書式を削除し、ナレッジをよりクリーンで解析しやすいものにします。",
    sideTipP4:
      "適切なチャンクとクリーニングはモデルのパフォーマンスを向上させ、より正確で価値のある結果を提供します。",
    previewTitle: "プレビュー",
    previewTitleButton: "プレビュー",
    previewButton: "Q&A形式に切り替える",
    previewSwitchTipStart:
      "現在のチャンクプレビューはテキスト形式です。質問と回答形式のプレビューに切り替えると、",
  ...
  stepThree: {
    ...
    creationContent:
      "ナレッジの名前は自動的に設定されましたが、いつでも変更できます。",
    label: "ナレッジ名",
    ...
    sideTipTitle: "次は何ですか",
    sideTipContent:
      "ドキュメントのインデックスが完了したら、ナレッジをアプリケーションのコンテキストとして統合することができます。プロンプトオーケストレーションページでコンテキスト設定を見つけることができます。また、独立したChatGPTインデックスプラグインとしてリリースすることもできます。",
    modelTitle: "埋め込みを停止してもよろしいですか？",
    modelContent:
      "後で処理を再開する必要がある場合は、中断した場所から続行します。",
  ...
  otherDataSource: {
    ...
    description:
      "現在、Difyのナレッジベースには利用できるデータソースが限られています。Difyのナレッジベースにデータソースを提供いただくことは、プラットフォームの柔軟性と能力を向上させる上で非常に有益です。貢献ガイドをご用意していますので、ぜひご協力ください。詳細については、以下のリンクをクリックしてください。",
    learnMore: "詳細はこちら",
  },
};
