const translation = {
  ...
  model: {
    params: {
      ...
      maxTokenSettingTip:
        "最大トークン設定が高いため、プロンプト、クエリ、およびデータのスペースが制限される可能性があります。現在のモデルの最大トークンの80%以下に設定してください。",
      setToCurrentModelMaxTokenTip:
        "最大トークンが現在のモデルの最大トークンの80%に更新されました {{maxToken}}.",
      stop_sequences: "停止シーケンス",
      stop_sequencesTip:
        "APIが進行中のトークンの生成を停止する最大4つのシーケンス。返されたテキストには停止シーケンスは含まれません。",
  ...
  menus: {
    ...
    plugins: "プラグイン",
    pluginsTips:
      "サードパーティのプラグインを統合するか、ChatGPT互換のAIプラグインを作成します。",
    datasets: "ナレッジ",
    datasetsTips:
      "近日公開：独自のテキストデータをインポートするか、Webhookを介してリアルタイムにデータを記述してLLMコンテキストを強化します。",
    newApp: "新しいアプリ",
    newDataset: "ナレッジの作成",
    tools: "ツール",
  ...
  account: {
    ...
    password: "パスワード",
    passwordTip:
      "一時的なログインコードを使用したくない場合は、永続的なパスワードを設定できます。",
    ...
    deletePrivacyLinkTip:
      "お客様のデータの取り扱い方法の詳細については、当社の",
    deletePrivacyLink: "プライバシーポリシー。",
    deleteSuccessTip:
      "アカウントの削除が完了するまでに時間が必要です。すべて完了しましたら、メールでお知らせします。",
  ...
  members: {
    ...
    inviteTeamMemberTip:
      "彼らはサインイン後、直接あなた様のチームデータにアクセスできます。",
    emailNotSetup:
      "メールサーバーがセットアップされていないので、招待メールを送信することはできません。代わりに招待後に発行される招待リンクをユーザーに通知してください。",
    email: "メール",
    ...
    invitationSentTip:
      "招待が送信され、彼らはDifyにサインインしてあなた様のチームデータにアクセスできます。",
  ...
  provider: {
    ...
    openaiHosted: {
      ...
      callTimes: "通話回数",
      usedUp:
        "トライアルクォータが使い果たされました。独自のモデルプロバイダを追加してください。",
    ...
    anthropicHosted: {
      ...
      close: "閉じる",
      trialQuotaTip:
        "お客様の Anthropic 試用枠は 2025/03/17 に失効し、その後は利用できなくなります。お早めにご利用ください。",
    },
    anthropic: {
      using: "埋め込み機能は使用中です",
      enableTip:
  ...
  modelProvider: {
    ...
    item: {
      deleteDesc:
        "{{modelName}}はシステムが推測するモデルとして利用されています。削除すると、一部の機能が使用不可能になる可能性があります。ご確認ください。",
      freeQuota: "無料のクォータ",
    },
    addApiKey: "APIキーを追加",
    invalidApiKey: "無効なAPIキー",
    ...
    providerManagedDescription:
      "モデルプロバイダによって提供される認証情報を使用します。",
    defaultConfig: "デフォルトの設定",
    apiKeyStatusNormal: "APIキーの状態は正常",
    apiKeyRateLimit:
      "レート制限に到達しました。{{seconds}}秒後に再度利用可能です",
    addConfig: "設定を追加",
    editConfig: "設定を編集",
    loadBalancingLeastKeyWarning:
      "負荷分散を利用するには、最低2つのキーを有効化する必要があります。",
    loadBalancingInfo:
      "デフォルトでは、負荷分散はラウンドロビン方式を採用しています。レート制限が発生した場合、1分間のクールダウン期間が適用されます。",
    upgradeForLoadBalancing:
      "負荷分散を利用するには、プランのアップグレードが必要です。",
  ...
  dataSource: {
    ...
    website: {
      title: "ウェブサイト",
      description:
        "ウェブクローラーを使ってウェブサイトからコンテンツを取り込みます。",
      with: "による",
  ...
  apiBasedExtension: {
    title:
      "API拡張機能は、Difyのアプリケーション全体での簡単な使用のための設定を簡素化し、集中的なAPI管理を提供します。",
  ...
  datasetMenus: {
    ...
    settings: "設定",
    emptyTip:
      "このナレッジはどのアプリケーションにも統合されていません。ドキュメントを参照してガイダンスを確認してください。",
    viewDoc: "ドキュメントを表示",
    relatedApp: "関連アプリ",
  ...
  imageUploader: {
    ...
    uploadFromComputerLimit:
      "アップロード画像のサイズは {{size}} MB を超えることはできません",
  ...
  fileUploader: {
    ...
    uploadFromComputerLimit:
      "アップロードファイルは{{size}}を超えてはなりません",
    uploadFromComputerUploadError:
      "ファイルのアップロードに失敗しました。再度アップロードしてください。",
    uploadFromComputerReadError:
      "ファイルの読み取りに失敗しました。もう一度やり直してください。",
